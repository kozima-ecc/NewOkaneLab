...
def get_sentiment_score(text):
    """ 簡易センチメント分析 """
    score = 0.0
    pos_words = ['上場', '買収', '提携', '設立', '開発', '発売', '最高', '黒字', '受賞', '採用', '開始', '成功', '増資', '好調']
    neg_words = ['赤字', '撤退', '中止', '延期', '不祥事', '不正', '処分', '辞任', '解散', '課徴金', '訴訟', '排除', '低迷']
    for w in pos_words:
...
def get_japanese_company_name(ticker):
    """ Yahoo!ファイナンスから日本語の正式名称を取得 """
    try:
        # YahooファイナンスURL (日本株も米国株も対応)
        ...
        # タイトルタグまたは特定のクラスから社名を取得
        # <h1 class="_2192-K1w">トヨタ自動車(株)</h1> のような構造
        h1 = soup.find('h1')
        if h1:
            raw_name = h1.get_text()
            # (株)などを除去
            name = raw_name.replace('(株)', '').replace('株)', '').replace('(', '').replace(')', '').strip()
            return name
    except:
        pass
    return None
...
def fetch_wiki_events(ticker, jp_company_name):
    ...
    try:
        ...
        # 404なら「株式会社」をつけて再トライ、それでもだめなら諦める
        ...
        # 抽出対象: <li>タグ (年表形式が多い) と <p>タグ
        target_texts = [tag.get_text() for tag in soup.find_all(['li', 'dd'])]
...
def main():
    print("🚀 企業沿革データ収集(検索エンジン不使用版)を開始します...")
    engine = get_db_engine()

    # 1. 既存データのクリア (News, History系)
    ...
    for i, row in df_comp.iterrows():
        ...
        # 1. Yahooファイナンスから日本語社名を取得
        jp_name = get_japanese_company_name(ticker)
        
        if not jp_name:
            # 取得できない場合はDBの名前を使用(ただし英語表記の場合はWikiヒット率低い)
            jp_name = db_name
